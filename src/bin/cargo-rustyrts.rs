...
fn main() {
    ...
    if  arg1 == "rustyrts" {
        ...
        match mode {
            ...
            Some(mode) => {
                // This arm is for when `cargo rustyrts (static|dynamic)` is called. We call `cargo build`,
...
fn select_and_execute_tests_dynamic() {
    ...
    println!(
        "#Tests with information: {}\n",
        analyzed_tests_names.iter().count()
    );
