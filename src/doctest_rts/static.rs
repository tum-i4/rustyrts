...
use crate::{
    callbacks_shared::{CRATE_ID, CRATE_NAME, PATH_BUF, PATH_BUF_DOCTESTS},
...
pub(crate) fn doctests_analysis(tcx: TyCtxt<'_>) {
    if let Some(path_doctests) = PATH_BUF_DOCTESTS.get() {
        ...
        if let Some(path) = PATH_BUF.get() {
            link_to_file(path.clone(), path_doctests.clone(), |buf| {
                init_path(buf, crate_name, crate_id, ENDING_GRAPH)
            });

            debug!(target = "doctests", "Linked graph for {}", crate_name);
        } else {
